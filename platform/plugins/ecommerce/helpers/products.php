...
use Botble\Ecommerce\Models\Wrapping;
...
if (!function_exists('get_related_products')) {
    ...
    function get_related_products($product, $limit = 4)
    {
        ...
        $data  = app(ProductInterface::class)->getProducts($params);
        $sub_categories = ProductCategory::where('parent_id', 74)->get();
        $wall_mural_sub_categories =  ProductCategory::where('parent_id', 66)->get();
        ...
        foreach ($data as $d) {
            ...
            $wall_display_products = $d->categories->whereIn('id', $all_categories);
            if ($wall_display_products->count() > 0) {
                $d->price = $d->price * ($d->wide * $d->height);
            }
            $all_wall_mural_categories  = array_merge([66], $wall_mural_sub_categories->pluck('id')->toArray());
            $wall_mural_products = $d->categories->whereIn('id', $all_wall_mural_categories);
            if ($wall_mural_products->count() > 0) {
                $d->price = $d->price * (100 * 100);
            }

            $wall_sticker_products = $d->categories->whereIn('id', $wall_sticker_categories);
...
if (!function_exists('get_wrapping_list')) {
    /**
     * @param int $limit
     * @return mixed
     */
    function get_wrapping_list()
    {
        return DB::table('wrappings')->where([
                'status' => BaseStatusEnum::PUBLISHED,
            ])->get();
    }
}
if (!function_exists('get_frame_list')) {
    /**
     * @param int $limit
     * @return mixed
     */
    ...
    {
        return DB::table('frames')->where([
                'status' => BaseStatusEnum::PUBLISHED,
            ])->get();
    }
}

if (!function_exists('get_material_list')) {
    /**
     * @param int $limit
     * @return mixed
     */
    ...
    {
        return DB::table('materials')->where([
                'status' => BaseStatusEnum::PUBLISHED,
            ])->get();
    }
}
