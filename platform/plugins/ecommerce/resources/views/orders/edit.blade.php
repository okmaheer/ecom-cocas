                                        ...
                                        <tbody>
                                            @foreach ($order->products as $orderProduct)
                                                ...
                                                @endphp
                                                    ...
                                                    @php
                                                    $defaultImage = RvMedia::getImageUrl($product->original_product->image, 'thumb', false, RvMedia::getDefaultImage());
                                                    $image = EcommerceHelper::get_order_single_image($order->id, $orderProduct->product_id, $defaultImage, $orderProduct->id);

                                                    $totalImages = EcommerceHelper::count_order_images($order->id, $orderProduct->product_id);
                                                    @endphp
                                                        <td class="width-60-px min-width-60-px vertical-align-t">
                                                            <div class="wrap-img"><a href="{{ $image }}" target="_blank"><img class="thumb-image thumb-image-cartorderlist" src="{{$image }}" alt="{{ $orderProduct->product_name }}"></a></div>
                                                            @if ($totalImages > 1)
                                                                <a href="{{ url('/dowload-order-images/'.$order->id.'/'.$orderProduct->product_id.'/'.$orderProduct->id.'/')}}" target="_blank" title="Download Images"><i class="fas fa-download" style="font-size: 23px; margin-top: 10px;"></i></a>
                                                            @endif
                                                        </td>
                                                    ...
                                                    <td class="pl5 p-r5 min-width-200-px">
                                                        ...
                                                        @if ($product)
                                                            ...
                                                            @endif
                                                            @if ($product->is_variation || $orderProduct->variation_attributes)
                                                                <p class="mb-0">
                                                                   <small>{{ $orderProduct->variation_attributes ?? $product->variation_attributes }}</small>
                                                                </p>
                                                            @endif
                                                        @endif
