...
@section('content')
    ...
    @if (Cart::instance('cart')->count() > 0)
        ...
        <style>
			.verifyBtn{float:right; font-size:11px;}
			.modal-backdrop{background-color: #000 !important;}
		</style>
        @php
        $sendOTPURL=url('/checkout/'.$token.'/send-otp');
        $verifyOTPURL=url('/checkout/'.$token.'/verify-otp');
        @endphp
        <script>
		function verifyPhone(phoneNumber, id){
        ...
        <div class="modal fade" id="verifyPhoneModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Verify Phone</h5>
              ...
              <div class="modal-body text-center">
                <form class="form-inline">
                  <div class="form-group mb-2">
                    <label for="staticEmail2" class="sr-only">OTP</label>
                    <input type="text" class="form-control" placeholder="Please enter recived OTP" maxlength="4" id="opt" value="">
                  </div>
                  <button type="button" class="btn btn-primary mb-2"  data-processing-text="Processing. Please wait..." data-error-header="Error" onclick="verifyOTP();">Verify OTP</button>
                </form>
              </div>
