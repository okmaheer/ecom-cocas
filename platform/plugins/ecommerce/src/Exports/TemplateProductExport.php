                                        ...
                                        ShouldAutoSize
{
    ...
    public function __construct(string $exportType = Excel::XLSX)
    {
		...
		$brand = count($this->brands) > 0?$this->brands->random():'';
        $product = [
            'name'                             => $productName,
            ...
            'wide'                             => '',
            'height'                           => '',
			'color'                            => 'Black,White,Brick Red',
			'is_direction'                      => Arr::random(['Yes', 'No']),
			'is_materials'                      => Arr::random(['Yes', 'No']),
			'is_frames'                      => Arr::random(['Yes', 'No']),
			'is_wrappings'                      => Arr::random(['Yes', 'No']),
        ];
    ...
    public function headings(): array
    {
        return [
			...
			'is_materials'                     => 'Material',
			'is_frames'                    	   => 'Frame',
			'is_wrappings'                     => 'Wrapping',
        ];
    }
        ...
        return [
            ...
            AfterSheet::class => function(AfterSheet $event) {
                ...
                for ($i = 2; $i <= $this->totalRow; $i++) {
                    ...
                    $event->sheet->getCell("{$withStorehouseManagementColumn}{$i}")->setDataValidation($booleanValidation);
                    $event->sheet->getCell("{$allowCheckoutWhenOutOfStockColumn}{$i}")->setDataValidation($booleanValidation);
					...
					$event->sheet->getCell("{$directionColumn}{$i}")->setDataValidation($booleanValidation);
					$event->sheet->getCell("{$materialColumn}{$i}")->setDataValidation($booleanValidation);
					$event->sheet->getCell("{$frameColumn}{$i}")->setDataValidation($booleanValidation);
					$event->sheet->getCell("{$wrappingColumn}{$i}")->setDataValidation($booleanValidation);
                }
            },
        ];
    }
    ...
    protected function getWholeNumberValidation($min = 0)
    {
        ...
        $validation->setPrompt(trans('plugins/ecommerce::bulk-import.export.template.prompt_whole_number', compact('min')));
    ...
    public function columnFormats(): array
    {
        ...
        return [
            ...
            'AC2:AC' . $this->totalRow => NumberFormat::FORMAT_GENERAL,
        ];
    }
