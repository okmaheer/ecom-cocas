...
class PublicCheckoutController
{
    ...
    /**
     * @var OrderProductImagesInterface
     */
    protected $orderProductImagesRepository;
     ...
     * @param OrderProductImagesInterface $orderProductImagesRepository
    ...
    public function __construct(
        ...
        OrderProductImagesInterface $orderProductImagesRepository,
        ...
        $this->orderProductImagesRepository = $orderProductImagesRepository;
    ...
    protected function processOrderData(string $token, array $sessionData, Request $request): array
    {
        ...
        if (!isset($sessionData['created_order_product'])) {
            ...
            foreach (Cart::instance('cart')->content() as $cartItem) {
                ...
                if(isset($cartItem->options['image']) && !empty($cartItem->options['image'])){
                    $actual_image_name = time().random_int(0,99999).".jpg";
                    $destination = public_path().'/images/orders/';
                    // dd($cartItem->options['image']);
                    if(!filter_var($cartItem->options['image'], FILTER_VALIDATE_URL)){
                    OrderHelper::createImage($cartItem->options['image'],$destination.'thumbs/'.$actual_image_name,'jpg');
                    }else{
                        \File::copy($cartItem->options['image'], $destination.'thumbs/'.$actual_image_name,'jpg');
                    }
                    //OrderHelper::createImage($cartItem->options['mainImage'],$destination.'main_images/'.$actual_image_name,'jpg');
                    $img_data = [
                        'order_id'     => $sessionData['created_order_id'],
                        'product_id'   => $cartItem->id,
                        'main_image' => $actual_image_name,
                        'crop_image' => '/images/orders/thumbs/'.$actual_image_name,
                    ];
                    $this->orderProductImagesRepository->create($img_data);
                }
            }
    ...
    public function postCheckout(
		...
		if ($request->input('address', [])) {
			$checkverify = $this->addressRepository
			->getModel()
			->where('customer_id', '=', auth('customer')->id())
			->where('phone', '=', $request->input('address.phone'))
			/*->where('phone_verify', '=', 1)*/
			->first();
			if(!$checkverify){
				/*return $response
                ->setError()
                ->setMessage(__('Please verify your phone number.'));*/
			}
		}
        ...
        if ($order) {
            ...
            foreach (Cart::instance('cart')->content() as $cartItem) {
                ...
                $orderProject = $this->orderProductRepository->create($data);

                if(isset($cartItem->options['mainImage']) && !empty($cartItem->options['mainImage'])){
                    $actual_image_name = time().random_int(0,99999).".jpg";
                    $destination = public_path().'/images/orders/';
                      if(!filter_var($cartItem->options['image'], FILTER_VALIDATE_URL)){
                    ...
                    $img_data = [
                        'order_id'     => $sessionData['created_order_id'],
                        'product_id'   => $cartItem->id,
                        'order_product_id'   => $orderProject->id,
                        'main_image' => $actual_image_name,
                        'crop_image' => $actual_image_name
                    ];
                    $this->orderProductImagesRepository->create($img_data);
                }

                #save canvas image
                if(isset($cartItem->options['canvasImage']) && !empty($cartItem->options['canvasImage'])){
                    $canvasImage = $cartItem->options['canvasImage'];
                    $img_canvas_data = [
                        'order_id'     => $sessionData['created_order_id'],
                        'product_id'   => $cartItem->id,
                        'order_product_id'   => $orderProject->id,
                        'main_image' => $canvasImage,
                        'crop_image' => $canvasImage
                    ];
                    $this->orderProductImagesRepository->create($img_canvas_data);
                    if (file_exists( public_path() . '/images/orders/temp/'.$canvasImage) && !empty($canvasImage)) {
                        rename(public_path().'/images/orders/temp/'.$canvasImage, public_path().'/images/orders/thumbs/'.$canvasImage);
                    }
                }

                #wall display products
                if(isset($cartItem->options['relImages']) && count($cartItem->options['relImages']) > 0){
                    $telatedImages = $cartItem->options['relImages'];
                    foreach($telatedImages as $key => $img){
                        $mainFile = $cartItem->options['canvasMainImage'][$key];
                        $img_data = [
                            'order_id'     => $sessionData['created_order_id'],
                            'product_id'   => $cartItem->id,
                            'order_product_id'   => $orderProject->id,
                        ...
                        $this->orderProductImagesRepository->create($img_data);
                        if (file_exists( public_path() . '/images/orders/temp/'.$img) && !empty($img)) {
                            rename(public_path().'/images/orders/temp/'.$img, public_path().'/images/orders/thumbs/'.$img);
                        }
                        if (file_exists( public_path() . '/images/orders/temp/'.$mainFile) && !empty($mainFile)) {
                            rename(public_path().'/images/orders/temp/'.$mainFile, public_path().'/images/orders/main_images/'.$mainFile);
                        }
                    }
                }
	...
	public function sendOTP($token,
        ...
        BaseHttpResponse $response){
			...
			->getModel()
			->where('customer_id', '=', auth('customer')->id())
			->where('phone', '=', $request->input('phoneNumber'))
			->where('phone_verify', '=', 1)
			->first();
			if(!$checkverify){
				$otpNo = mt_rand(0000, 9999);
				$message = 'Your OTP to verify phone number Cocas is '.$otpNo.'. Do not share it with anyone. Contact Us if not initiated by you.';
				EcommerceHelper::sendSms($request->input('phoneNumber'), $message );
				$this->addressRepository->update([['customer_id', '=', auth('customer')->id()]], ['phone_otp' => $otpNo, 'phone_verify' => 0, 'phone' =>  $request->input('phoneNumber')]);
				echo 'success'; die;
			}else{
				echo 'error'; die;
			}
	...
	public function verifyOTP($token,
        Request $request,
        BaseHttpResponse $response){
			$otpCheck = $this->addressRepository
			->getModel()
			->where('customer_id', '=', auth('customer')->id())
			->where('phone_otp', $request->input('otp'))
			->first();
			if($otpCheck){
				$this->addressRepository->update([['id', '=', $otpCheck->id]], ['phone_otp' => '', 'phone_verify' => 1]);
				return $response
				->setMessage(__('success'));
			}else{
				return $response
				->setError()
				->setMessage(__('Please enter valid OTP'));
			}
	}
}
