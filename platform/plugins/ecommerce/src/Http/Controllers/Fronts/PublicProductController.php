...
class PublicProductController
{
    ...
    public function getProduct($slug, Request $request)
    {
        ...
        if ($i != 0) {
            rsort($parents);
            $category_id = $parents[$i - 1];
        }


        //product - Single Canvas Print
        if ($product->id == 114) {

            // dd($category_id);
            return Theme::scope(
                'ecommerce.custom-product-pages.single-canvas-print',
		...
		//product - Large Canvas Print
        if ($product->id == 537) {

            // dd($category_id);
            return Theme::scope(
                'ecommerce.custom-product-pages.large-canvas-print',
                compact('product', 'selectedAttrs', 'productImages', 'variationDefault', 'productVariation'),
                'plugins/ecommerce::themes.product'
            )
                ->render();
        ...
        if ($category_id == 43) {
            ...
            // dd($category_id);
            return Theme::scope(
                'ecommerce.category-product-pages.wall-sticker-product',
                compact('product', 'selectedAttrs', 'productImages', 'variationDefault', 'productVariation'),
                'plugins/ecommerce::themes.product'
            )
                ->render();
        ...
        if ($category_id == 39) {
            // dd($category_id);
            return Theme::scope(
                'ecommerce.category-product-pages.split-photo-collage-product',
                compact('product', 'selectedAttrs', 'productImages', 'variationDefault', 'productVariation'),
                'plugins/ecommerce::themes.product'
            )
                ->render();
        ...
        if ($category_id == 40) {
            ...
            // dd($category_id);
            return Theme::scope(
                'ecommerce.custom-product-pages.rolled-canvas-print',
                compact('product', 'selectedAttrs', 'productImages', 'variationDefault', 'productVariation'),
                'plugins/ecommerce::themes.product'
            )
                ->render();
        ...
        if ($category_id == 43) {
            return Theme::scope(
                'ecommerce.category-product-pages.wall-mural-product',
                compact('product', 'selectedAttrs', 'productImages', 'variationDefault', 'productVariation'),
                'plugins/ecommerce::themes.product'
            )
                ->render();
        ...
        if ($category_id == 44) {
            return Theme::scope(
                'ecommerce.category-product-pages.roller-blind-product',
                compact('product', 'selectedAttrs', 'productImages', 'variationDefault', 'productVariation'),
                'plugins/ecommerce::themes.product'
            )
                ->render();
        ...
        if ($category_id == 38) {
            return Theme::scope(
                'ecommerce.category-product-pages.mini-sticker-product',
                compact('product', 'selectedAttrs', 'productImages', 'variationDefault', 'productVariation'),
                'plugins/ecommerce::themes.product'
            )
                ->render();
        ...
        if ($category_id == 46) {
            return Theme::scope(
                'ecommerce.category-product-pages.window-print-product',
                compact('product', 'selectedAttrs', 'productImages', 'variationDefault', 'productVariation'),
                'plugins/ecommerce::themes.product'
            )
                ->render();
    ...
    public function getProductCategory(
		...
		if ($category->id == 55) {
			return redirect()->to('/products/large-canvas-print');
		}
        ...
        if ($category->id == 42) {
            $pslug = Str::slug($products[0]->name);
            ...
            // Wall Sticker 42
            if ($category_id == 42) {
				return redirect()->to('/products/single-canvas-print');
                return Theme::scope(
                    'ecommerce.category-product-pages.wall-sticker-product',
                    compact('product', 'selectedAttrs', 'productImages', 'variationDefault', 'productVariation'),
                    'plugins/ecommerce::themes.product'
    ...
    public function getOrderTracking(Request $request, OrderInterface $orderRepository)
    {
        ...
        Theme::breadcrumb()->add(__('Home'), route('public.index'))
            ->add(
                __('Order tracking :code', ['code' => $code ? ' #' . $code : '']),
    ...
    public function cropImages(Request $request)
    {
        dd('here');
        $actual_image_name = '';
        if(!empty($request->input('cropImg'))){
            $cropImg = $request->input('cropImg');
            $actual_image_name = time().random_int(0,99999).".jpg";
            if($request->input('type') == 'canvas' && !empty(trim($request->input('canvasName')))){
                $actual_image_name = $request->input('canvasName');
            }
            if($request->input('type') == 'mainImage'){

                $actual_image_name = 'main_'.$actual_image_name;
                if(!empty(trim($request->input('mainImageName')))){
                    $actual_image_name = $request->input('mainImageName');
                }
            }
            $destination = public_path().'/images/orders/';
            OrderHelper::createImage($cropImg,$destination.'temp/'.$actual_image_name,'jpg');
        }
        echo($actual_image_name);die;
    }

    public function dowloadOrderImages($orderID, $productID, $orderProductID, Request $request){

        if($orderID > 0 && $productID > 0){
            $orderimages = EcommerceHelper::get_order_images($orderID, $productID, $orderProductID);
            $files = array();
            if(count($orderimages) > 0){
                foreach($orderimages as $myImages){
                    if (file_exists( public_path() . '/images/orders/thumbs/'.$myImages->crop_image)) {
                        $files[] = public_path('images/orders/thumbs/'.$myImages->crop_image);
                    }
                    if (file_exists( public_path() . '/images/orders/main_images/'.$myImages->main_image)) {
                        $files[] = public_path('images/orders/main_images/'.$myImages->main_image);
                    }
                }
                $zip = new ZipArchive;
                $fileName = $orderID.'-'.$productID.'-'.$orderProductID.'.zip';
	...
	public function updateCustomAttributesPrice(Request $request)
    {

		$product = DB::table('ec_products')->where([ 'id' => $request->input('productID') ])->first();
        $price = $product->price;

		#calculate SIZE price
		if(!empty($request->input('sizeData')) && $request->input('sizeData') != 'undefiend'){

			$productSizes = $request->input('sizeData');
		...
		if(!empty($request->input('framePrice')) && $request->input('framePrice') != 'undefiend'){
			$framePrice = $request->input('framePrice');
			$selectedWidth = $request->input('selectedWidth');
			$selectedHeight = $request->input('selectedHeight');
		...
		if(!empty($request->input('materialPrice')) && $request->input('materialPrice') > 0){
			$materialPrice = $request->input('materialPrice');
			$selectedWidth = $request->input('selectedWidth');
			$selectedHeight = $request->input('selectedHeight');
			$materialPrice = number_format(floatval($selectedWidth)*floatval($selectedHeight)*floatval($materialPrice), 2, '.', '');
			$price = round($materialPrice)+$price;
		}

		echo round($price);die;
    }

	public function updateWallMuralPrice(Request $request)
    {
		$product = DB::table('ec_products')->where([ 'id' => $request->input('productID') ])->first();
        $price = $product->price;
		#calculate SIZE price
		$baseWidth = floatval($request->input('selectedWidth'));
		$baseHeight = floatval($request->input('selectedHeight'));
		$basePrice = $price;
		$price = number_format(floatval($baseWidth)*floatval($baseHeight)*floatval($basePrice), 2, '.', '');

		#calculate MATERIAL price
		if(!empty($request->input('materialPrice')) && $request->input('materialPrice') > 0){
			$materialPriceData = $request->input('materialPrice');
			$materialPrice = number_format(floatval($baseWidth)*floatval($baseHeight)*floatval($materialPriceData), 2, '.', '');
			$price = round($materialPrice)+$price;

			if(!empty($request->input('coverEntireWall')) && $request->input('coverEntireWall') == 'Yes' ){
				$coverPrice = setting('cover_entire_wall');
				$coverWallPrice = number_format(floatval(floatval($baseWidth)*floatval($coverPrice))*floatval($materialPriceData), 2, '.', '');
				$price = number_format(round($coverWallPrice))+$price;
			}
		}
		echo round($price);die;
    }
