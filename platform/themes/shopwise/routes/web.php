...
Route::group(['namespace' => 'Theme\Shopwise\Http\Controllers', 'middleware' => ['web', 'core']], function () {
    Route::group(apply_filters(BASE_FILTER_GROUP_PUBLIC_ROUTE, []), function () {
		...
		Route::get('ajax/meterials', 'ShopwiseController@ajaxGetMeterials')
            ->name('public.ajax.meterials');
...
Route::group(['namespace' => 'Theme\Shopwise\Http\Controllers', 'middleware' => ['web', 'core']], function () {
    Route::group(apply_filters(BASE_FILTER_GROUP_PUBLIC_ROUTE, []), function () {
        ...
        // Route::get('/', 'ShopwiseController@getIndex')->name('public.index');

        // Route::get('sitemap.xml', 'ShopwiseController@getSiteMap')->name('public.sitemap');

        // Route::get('{slug?}' . config('core.base.general.public_single_ending_url'), 'ShopwiseController@getView')->name('public.single');
        Route::get('{slug?}' . config('core.base.general.public_single_ending_url'), function () {
            echo config('core.base.general.public_single_ending_url');
        });
    });
});
