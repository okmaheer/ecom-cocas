...
<div class="section">
    <div class="container">
        <div class="row">
            <div class="col-lg-8 col-md-5 mb-4 mb-md-0">

                <div class="product-image">
                    <div class="product_img_box" style="height:100%">

                        @if ($product->frame_type !="")
                            @include(Theme::getThemeNamespace() . '::views/ecommerce/product-templates/'.$product->frame_type)
                        @endif
                    ...
                    <div class="product_description">
                        ...
                        <div class="cart_extra">
                            <form class="add-to-cart-form" method="POST" action="{{ route('public.cart.add-to-cart') }}">
                                ...
                                @for ($i = 1; $i <= $product->frame_size; $i++)
                                    <input type="hidden" name="canvasCropImage[]" id="uploaded_crop_image_0{{ $i }}" value="">
                                    <input type="hidden" name="canvasMainImage[]" id="uploaded_main_image_0{{ $i }}" value="">
                                @endfor
...
<script>
    ...
    $(document).ready(function() {
        ...
        $(".uploadImage").click(function(){
            ...
            $('#cropID').val(dataID);
            var imageWidth = $(".img_box_"+dataID).width();
            var imageHeight = $(".img_box_"+dataID).height();
            imageWidth = Math.round(imageWidth);
            imageHeight = Math.round(imageHeight);

            $("#canvasSize").val(parseFloat(imageWidth)+' / '+parseFloat(imageHeight));
        ...
        $("#upload_image").change(function(event) {
            ...
            if(ext == "jpg" || ext == "jpeg" || ext == "JPG" || ext == "JPEG" || ext == "png" || ext == "PNG"){
                ...
                var done = function(url) {
                    ...
                    var canvSize = $("#canvasSize").val();
                    canvSize = canvSize.split('/');
                    cropper = new Cropper(image, {
                        aspectRatio: canvSize[0].trim() / canvSize[1].trim(),
                        viewMode: 3,
                        //preview: '.preview'
                    });
                };
                ...
                if (files && files.length > 0) {
                    ...
                    reader.onload = function(event) {
                        ...
                        var imgID = $('#cropID').val();
                        var cropImg = reader.result;
                        convertImage(cropImg, imgID, 'mainImage')
                    };
        ...
        $('#crop').click(function() {
            ...
            canvas = cropper.getCroppedCanvas({
                width: canvSize[0].trim(),
                height: canvSize[1].trim(),
            });
            ...
            $("#uploaded_image_"+imgID).attr('src', dataurl);
            ...
            canvas.toBlob(function(blob) {
                ...
                reader.onloadend = function() {
                    ...
                    //$("#uploaded_crop_image_"+imgID).val(base64data);
                    // console.log(base64data);

                    convertImage(base64data, imgID, 'frame');
                }
            });
        ...
        html2canvas(document.getElementById("main_frame"),
            {
                ...
                anchorTag.download = "canvas_file.jpg";
    ...
    var ajaxRequest = 0;
    function convertImage(cropImg, imgID, type){
        if(type != 'canvas'){ setCanvasUrl(); }
        $.ajaxSetup({headers:{"X-CSRF-TOKEN":$('meta[name="csrf-token"]').attr("content")}});
        var canvasName = $('#canvasImg').val();
        var mainImageName = $('#uploaded_main_image_'+imgID).val();
        setTimeout(function(){
            ajaxRequest = parseInt(ajaxRequest)+1;
			if (type == 'frame') { $('.preloader').show().css({'background':'rgba(255, 255, 255, 0.7)'});}
            $(".btn-pro-addtocart").prop('disabled', true);
            $("#btn-pro-addtocart").html('<i class="icon-basket-loaded"></i> Loading...').prop('disabled', true);
            $.ajax({
        		type: 'POST',
        		url: '{{ $cropImgUrl }}',
        		data: {cropImg:cropImg,imgID:imgID, type:type, canvasName:canvasName,mainImageName:mainImageName},
        		success: function(img){
                    if(type == 'canvas'){
                        $('#canvasImg').val(img);
                    }else if(type == 'mainImage'){
                        $("#uploaded_main_image_"+imgID).val(img);
                    }else{
                        $("#uploaded_crop_image_"+imgID).val(img);
                    }
                    ajaxRequest = parseInt(ajaxRequest)-1;
                    setTimeout(function () {
